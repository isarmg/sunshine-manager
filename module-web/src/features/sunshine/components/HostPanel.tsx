...
function PairingSection({ host, canWrite }: { host: SunshineHostInfo; canWrite: boolean }) {
    ...
    <section className="section-band">
      ...
      <div className="sunshine-pin-form">
        ...
        <label className="inline-field"><span>设备名称</span>
          <input value={deviceName} maxLength={80} disabled={!canWrite} onChange={(event) => setDeviceName(event.target.value)} placeholder="Moonlight Client" />
        </label>
...
function ConfigSection({ host, canWrite }: { host: SunshineHostInfo; canWrite: boolean }) {
  const queryClient = useQueryClient();
    ...
    <section className="section-band">
      <SectionHeader
        ...
        actions={editMode ? (
          ...
          <ActionButton icon={Edit2} label="编辑 JSON" disabled={!canWrite || !query.data} onClick={() => setDraft(JSON.stringify(query.data ?? {}, null, 2))} />
        )}
      />
...
function SystemSection({ host, canWrite }: { host: SunshineHostInfo; canWrite: boolean }) {
    ...
    <div className="sunshine-host-panel">
      ...
      {HOST_SECTIONS.map(({ key }) => (
        <div
          ...
          hidden={section !== key}
        >
          {section === "apps" && key === "apps" ? <AppsSection host={host} canWrite={canWrite} /> : null}
          {section === "clients" && key === "clients" ? <ClientsSection host={host} canWrite={canWrite} /> : null}
          {section === "pairing" && key === "pairing" ? <PairingSection host={host} canWrite={canWrite} /> : null}
          {section === "config" && key === "config" ? <ConfigSection host={host} canWrite={canWrite} /> : null}
          {section === "system" && key === "system" ? <SystemSection host={host} canWrite={canWrite} /> : null}
        </div>
      ))}
    </div>
  );
