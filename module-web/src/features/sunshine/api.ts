...
const sunshineHostPath = (id: string) => `/hosts/${pathSegment(id)}`;
...
export function parseSunshineAppsResponse(value: unknown): SunshineAppsResponse {
...
export function parseSunshineClientsResponse(value: unknown): SunshineClientsResponse {
  if (
    !isRecord(value)
...
export const sunshineApi = {
  sunshineHosts: (signal?: AbortSignal) => request<SunshineHostInfo[]>("/hosts", { signal }),
