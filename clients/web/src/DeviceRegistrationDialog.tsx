...
export function TicketPanel({ticket}:{ticket:Ticket}){
 ...
 <p>{t("配置 管理端 的 WSS 地址和可信 CA、本机 Sunshine 的 HTTPS 地址及凭据后，按安装手册运行 sunshine-agent init，再启动 客户端 服务。管理端 不接收 Sunshine 密码。", "Configure the manager WSS address and trusted CA, and the local Sunshine HTTPS address and credentials. Follow the installation guide to run sunshine-agent init, then start the agent service. The manager never receives the Sunshine password.")}</p></section>;
}
