#!/usr/bin/env python3
"""Build a source-bound Agent archive on its native OS; never package the Server."""
import argparse
import hashlib
import json
import os
...
def main():
    ...
    version = tomllib.loads((ROOT / "agent/Cargo.toml").read_text())["package"]["version"]
    if not re.fullmatch(r"[0-9]+\.[0-9]+\.[0-9]+(?:-rc\.[0-9]+)?", version):
        parser.error("unsafe Agent version")
    if args.require_tag:
        tag = f"agent-v{version}"
        if run("git", "cat-file", "-t", tag) != "tag" or run("git", "rev-parse", tag + "^{commit}") != sha:
            parser.error("annotated Agent tag must resolve to this exact source")
    ...
    executable = "sunshine-agent.exe" if windows else "sunshine-agent"
    env = dict(os.environ, SUNSHINE_AGENT_BUILD_SHA=sha, CARGO_INCREMENTAL="0")
    if windows:
        env["RUSTFLAGS"] = env.get("RUSTFLAGS", "") + " -C target-feature=+crt-static"
    subprocess.run(["cargo", "build", "--locked", "--release", "-p", "sunshine-agent", "--target", target], cwd=ROOT, env=env, check=True)
    ...
    expected = f"sunshine-agent {version} (git {sha}; sunshine-management/1)"
    if run(str(binary), "--version") != expected:
        raise RuntimeError("binary source identity mismatch")
    name = f"sunshine-agent-{version}-{target}"
    with tempfile.TemporaryDirectory(prefix="agent-package-") as tmp:
        stage = Path(tmp) / name
        stage.mkdir()
