...
| SUN-079 | Server Rust 固定 Foundation `=0.7.0` + 完整 revision `77e7ad7af8e1bf62432bd6bdd8fa9aff54cb39d1`；八个 Web 包固定同版正式 Release tarball 与 lockfile integrity | `Cargo.toml`、`web/package.json`、lockfiles、toolchain check | 开发运维 | 中 | path/file、可变分支或多版本混用破坏独立供应链；构建通过不代表主分支已发布 | 独立 CI；Cargo locked resolve、`npm ci`、URL/version/integrity 与无 sibling 来源检查 |
