...
```text
/opt/isarmg/sunshine-manager/releases/0.10.2/  root-owned read-only release
...
```text
/opt/isarmg/sunshine-manager/releases/0.10.2/bin/sunshine-manager \
  serve-release --root /opt/isarmg/sunshine-manager/releases/0.10.2
```
...
从干净且 annotated `v0.10.2` 精确指向 HEAD 的 checkout：
...
```bash
tar -xzf sunshine-manager-0.10.2-x86_64-unknown-linux-gnu.tar.gz \
  -C /opt/isarmg/sunshine-manager/releases
/opt/isarmg/sunshine-manager/releases/0.10.2/bin/sunshine-manager \
  verify-release --root /opt/isarmg/sunshine-manager/releases/0.10.2
```
...
sunshine-manager verify-release --root /opt/isarmg/sunshine-manager/releases/0.10.2
...
系统信任库；Windows LocalSystem 使用计算机信任存储。详见[简化配对](https://github.com/isarmg/sunshine-manager-server/blob/v0.10.2/docs/simple-pairing.md)。
...
backup/verify/restore，并未声明支持本仓当前 0.10.2。不能因依赖更新或构建通过推定备份可跨版本恢复；
