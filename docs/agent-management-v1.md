# Sunshine Agent 管理改造（实施中，尚未发布）

本文件记录新管理链路的边界、已经实现的执行内核及尚未完成的接入项，**不是完成验收声明**。
当前 Manager 服务尚未切换为 Agent 模式；本次没有删除、迁移或修改已有实例数据库。
切换完成时应删除旧直连执行代码，而不是提供兼容模式。数据转换、备份恢复、升级仍属于 `sarmg-upgrade`。
