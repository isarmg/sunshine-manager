//! Product WSS transport. No generic Agent platform, URL passthrough or Manager credential logging.
use crate::{
    adapter::Sunshine,
...
use sunshine_agent_protocol::{
    AgentMessage, Binding, Capabilities, ConfigSnapshot, MAX_MESSAGE_BYTES, ManagerMessage,
...
pub const CONNECT_PATH: &str = "/sunshine-agent/v1/connect";
...
pub enum TransportError {
    #[error("invalid Agent WSS configuration")]
    Configuration,
    #[error("Agent credential rejected or revoked")]
    Revoked,
        ...
        loop {
            tokio::select! {
                ...
                Some((operation_id, report)) = executing.next(), if !executing.is_empty() => {
                    in_flight = None;
                    send(&mut socket, AgentMessage::Result { operation_id, report }).await?;
                },
                _ = tick.tick() => {
                    ...
                    send(&mut socket, AgentMessage::Heartbeat {
...
async fn send(socket: &mut Socket, message: AgentMessage) -> Result<(), TransportError> {
