//! Local provisioning storage, never an Agent command surface.
#[cfg(target_os = "linux")]
use std::path::Path;

#[derive(Debug, thiserror::Error)]
#[error("protected Agent state unavailable; check local ownership, permissions and integrity")]
...
#[cfg(target_os = "linux")]
pub fn prepare_root(path: &Path) -> Result<sarmg_agent_fs_safety::PrivateDirectory, StorageError> {
    sarmg_agent_fs_safety::PrivateDirectory::create(path).map_err(storage_error)
}
...
pub struct ProtectedState {
    directory: sarmg_agent_fs_safety::PrivateDirectory,
    _lock: sarmg_agent_fs_safety::AdvisoryLock,
}
#[cfg(target_os = "linux")]
impl ProtectedState {
    pub fn open(path: &Path) -> Result<Self, StorageError> {
        use sarmg_agent_fs_safety::{AdvisoryLock, EntryName, PrivateDirectory};
    ...
    pub fn read(&self, name: &str) -> Result<Option<Vec<u8>>, StorageError> {
        use sarmg_agent_fs_safety::{EntryName, InventoryLimits};
    ...
    pub fn put(&self, name: &str, bytes: &[u8]) -> Result<(), StorageError> {
        use sarmg_agent_fs_safety::{AtomicFile, EntryName};
    ...
    pub fn import_bootstrap(&self, path: &Path) -> Result<(), StorageError> {
        use sarmg_agent_fs_safety::{EntryName, PrivateDirectory};
