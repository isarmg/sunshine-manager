...
use sarmg_agent_secure_http::{
    Certificate, NetworkPolicy, ResponseBudget, SecureHttpClient, TlsConfig, Url,
};
use serde_json::Value;
use sha2::{Digest, Sha256};
use sunshine_agent_protocol::{ConfigSnapshot, Effectiveness, SUNSHINE_VERSION, config::FIELDS};
...
/// Full configuration stays on the Agent. Deliberately not Debug or Serialize.
...
impl Configuration {
    ...
    pub(crate) fn merge(
        &self,
        set: &BTreeMap<String, sunshine_agent_protocol::config::FieldValue>,
        remove: &std::collections::BTreeSet<String>,
    ) -> Result<Self, AdapterError> {
        let mut merged = self.clone();
        for (key, value) in set {
            sunshine_agent_protocol::config::validate_field(key, value)
...
impl LocalSunshine {
    pub fn new(
        ...
        let client = SecureHttpClient::new(
            ...
            format!("sunshine-agent/{}", env!("CARGO_PKG_VERSION")),
        )
