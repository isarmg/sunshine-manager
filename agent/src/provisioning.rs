...
use sunshine_agent_protocol::{
    AgentOs, Binding, Capabilities, Effectiveness, PROTOCOL, SUNSHINE_VERSION, config::FIELDS,
};
...
pub enum ProvisionError {
    ...
    #[error("Agent execution journal unavailable")]
    Journal,
}
...
async fn provision(store: &ProtectedState) -> Result<Identity, ProvisionError> {
    ...
    endpoint.set_path("/sunshine-agent/v1/identity");
    ...
    let binding: Binding = if response.status.is_success() {
        ...
        endpoint.set_path("/sunshine-agent/v1/enroll");
