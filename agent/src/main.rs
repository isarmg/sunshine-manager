...
use sunshine_agent::{provisioning, storage};
...
fn execute(args: &[std::ffi::OsString]) -> Result<(), String> {
    if args.len() < 3 || args[1] != "--state" {
        return Err("Usage: sunshine-agent init --state ABSOLUTE_PATH --bootstrap PROTECTED_FILE | run --state ABSOLUTE_PATH | --version".into());
    }
    let path = PathBuf::from(&args[2]);
    if !path.is_absolute() {
        return Err("Agent state path must be absolute".into());
    }
    if args[0] == "init" && args.len() == 5 && args[3] == "--bootstrap" {
        ...
        println!(
            "Protected bootstrap imported; registration occurs when Agent starts. Remove the original bootstrap after successful registration."
        );
        return Ok(());
    }
                ...
                #[cfg(unix)]
                {
                    let mut term =
                        tokio::signal::unix::signal(tokio::signal::unix::SignalKind::terminate())
                            .map_err(|_| "Agent signal handler unavailable")?;
