...
async fn real_https_patch_preserves_original_configuration_and_filters_metadata() {
    ...
    let task = Task {
        ...
        command: Command::PatchConfig {
            expected_revision: sunshine_agent::adapter::Configuration::from_response(config("28"))
                .unwrap()
                .revision(),
            set: std::collections::BTreeMap::from([(
                "qp".into(),
                sunshine_agent_protocol::config::FieldValue::Integer(29),
            )]),
...
async fn authenticated_wss_delivers_result_and_revocation_stops_reconnect() {
    ...
    let task = Task {
        ...
        command: sunshine_agent_protocol::Command::ReadConfig {},
    };
    let manager = tokio::spawn(async move {
        ...
        assert!(matches!(
            serde_json::from_str::<AgentMessage>(&hello).unwrap(),
            AgentMessage::Hello { .. }
        ));
        socket
            .send(Message::Text(
        ...
        while !got_result || !got_health {
            match socket.next().await.unwrap().unwrap() {
                Message::Text(message) => {
                    match serde_json::from_str::<AgentMessage>(&message).unwrap() {
                        AgentMessage::Result {
                            operation_id,
                            report,
                        } => {
    ...
    let capabilities = Capabilities {
        ...
        os: AgentOs::LinuxX86_64,
        sunshine_version: SUNSHINE_VERSION.into(),
        restart_allowed: false,
        managed_fields: sunshine_agent_protocol::config::FIELDS
            .iter()
...
fn wss_endpoint_policy_rejects_plaintext_and_credentials_in_urls() {
    use sarmg_agent_secure_http::Url;
    use sunshine_agent::transport::validate_manager_endpoint;
    for endpoint in [
        "ws://manager.example/sunshine-agent/v1/connect",
        "https://manager.example/sunshine-agent/v1/connect",
        "wss://token@manager.example/sunshine-agent/v1/connect",
        "wss://manager.example/sunshine-agent/v1/connect?token=secret",
    ...
    assert!(
        validate_manager_endpoint(
            &Url::parse("wss://manager.example/sunshine-agent/v1/connect").unwrap()
        )
        .is_ok()
    );
}
